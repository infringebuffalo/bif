...
if ($id != 0)
    ...
    if (hasPrivilege('scheduler'))
        {
        $pageDescription .= "&nbsp;&nbsp;<a href='editCategory.php?id=$id'>[edit category]</a>\n";
...
if ($id != 0)
    {
    $stmt = dbPrepare('select proposal.id, proposerid, name, title, orgfields, submitted, access from proposal join user on proposerid=user.id join proposalCategory on proposal.id=proposalCategory.proposal_id where proposalCategory.category_id=? and deleted=0 order by title');
    $stmt->bind_param('i',$id);
    }
else
    {
